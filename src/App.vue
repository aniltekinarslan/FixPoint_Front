...
export default {
	...
	computed: {
		navPos() {
			if (
					this.innerWidth <= 768 &&
					(this.$store.getters["app/navPos"] === "top" ||
							this.$store.getters["app/navPos"] === "bottom")
			) {
				return "left";
			}
		...
		viewAnimation() {
			return this.$store.getters["app/viewAnimation"] || "none";
		},
		isLogged() {
			return this.$store.getters["app/isLogged"];
		},
		splashScreen() {
			return this.$store.getters["app/splashScreen"];
		}
	},
	methods: {
		resizeOpenNav() {
...
.el-table .selectedRow {
	background: #cbedf5 !important;
}

.el-table .selectedCell {
...
.input-blue .el-input__inner {
	background-color: #c9faff;
}
.input-red .el-input__inner {
	background-color: #ffc9c9;
}
.input-orange .el-input__inner {
